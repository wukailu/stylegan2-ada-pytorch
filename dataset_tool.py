...
def open_ImageNet(datapath: str, *, max_images: Optional[int]):
    from torchvision.datasets import ImageFolder
    from torchvision import transforms
    ds = ImageFolder(root=datapath)
    max_idx = maybe_min(len(ds), max_images)
    print('max_idx = ', max_idx)

    def iterate_images():
        for idx, (img, label) in enumerate(ds):
            yield dict(img=np.asarray(img), label=int(label))
            if idx >= max_idx - 1:
                break
